...
void Timer0ACapture_init(void)
{
    SYSCTL->RCGCTIMER |= (1<<0);     /* enable clock to Timer Block 0; please refer to page 339 of the datasheet*/
    SYSCTL->RCGCGPIO |= (1<<1);      /* enable clock to PORTB; please refer to page 349 of the datasheet*/
    GPIOB->DIR &= (1<<6) ;        /* make PB6 an input pin; please refer to page 663 of the datasheet*/
    GPIOB->DEN |= (1<<6);         /* make PB6 as digital pin; please refer to page 682 on the datasheet */
...
*/
    ...
    TIMER0->CTL &= ~(1ul <<0) ;          /* disable timer0A during setup;  */
    TIMER0->CFG = 0x04;            /* 16-bit timer mode */
    TIMER0->TAMR = (1<<4) | (1<<2) | (3<<0);        /* up-count, edge-time, capture mode */
    TIMER0->CTL |= (1<<2)| (1<<3);        /* capture the rising edge */
